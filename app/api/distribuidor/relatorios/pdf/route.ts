...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .faixa { background: #FFD700; height: 3mm; }
    .content { padding: 8mm 15mm; }
    .meta { font-size: 7pt; color: #6b7280; margin-bottom: 6mm; }
    table { width: 100%; border-collapse: collapse; font-size: 7.5pt; }
    thead tr { background: #1B1B8E; color: #fff; }
    thead th { padding: 4px 6px; text-align: left; font-weight: 600; white-space: nowrap; }
    tbody tr:nth-child(even) { background: #f3f4f6; }
    tbody td { padding: 3.5px 6px; border-bottom: 1px solid #e5e7eb; white-space: nowrap; }
...
<body>
  ...
  <div class="footer">
    <span>Recife Cap — Relatório do Distribuidor · Confidencial</span>
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      return new NextResponse(pdfBuffer as any, {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="recife-cap-dist-${slug}.pdf"`,
        },
      })
    } finally {
      await browser.close()
    ...
    console.error('[distribuidor/relatorios/pdf]', e)
